...
export default function HotelWalkthrough() {
  ...
  // Pause WebGL rendering when walkthrough section is out of viewport to preserve 60fps performance
  useEffect(() => {
  ...
  // Scroll listener tracking progress through the 3D walkthrough track via rAF
  useEffect(() => {
    let rafWalkthroughId = null;

    const handleScroll = () => {
      if (rafWalkthroughId) return;
      rafWalkthroughId = requestAnimationFrame(() => {
        if (!containerRef.current) {
          rafWalkthroughId = null;
          return;
        }
    ...
    return () => {
      ...
      if (rafWalkthroughId) cancelAnimationFrame(rafWalkthroughId);
    };
  }, [rooms.length]);
  ...
  const isCurrentRoomLit = Boolean(roomLights[currentRoom.id]);
      ...
      >
        ...
        <div className="scene-caption-container container" style={{ pointerEvents: 'none' }}>
          <div className="scene-caption-card" style={{ pointerEvents: 'auto' }}>
            ...
            {/* Interactive Light Switch Button & Tactile Controls */}
            <div className="caption-actions-row">
              <button
                type="button"
                onClick={() => toggleRoomLight(currentRoom.id)}
                className={`btn-walkthrough-lamp ${isCurrentRoomLit ? 'lamp-active' : ''}`}
                aria-label="სანათის გადართვა"
              >
                <span className="lamp-btn-icon">{isCurrentRoomLit ? '💡' : '🕯️'}</span>
                <span className="lamp-btn-text">
                  {isCurrentRoomLit ? 'სანათის გამორთვა' : 'სანათის ჩართვა'}
                </span>
                <span className="lamp-status-dot" />
              </button>
              <span className="lamp-hint-label">შეეხეთ სანათს 3D სივრცეში ან ღილაკს</span>
            </div>
          </div>
        </div>

        {/* Side Navigation Dots for Desktop */}
        <aside className="walkthrough-navigation desktop-only" aria-label="3D სივრცეების ნავიგაცია">
        ...
        {/* Mobile Dedicated Horizontal Room Switcher Bar */}
        <div className="walkthrough-mobile-bar" aria-label="ოთახების სწრაფი არჩევა">
          {rooms.map((room, idx) => (
            <button
              key={room.id}
              onClick={() => scrollToRoom(idx)}
              className={`walkthrough-mobile-tab ${activeRoomIndex === idx ? 'tab-active' : ''}`}
              aria-label={room.name}
            >
              <span className="mobile-tab-num">0{idx + 1}</span>
              <span className="mobile-tab-name">{room.name}</span>
            </button>
          ))}
        </div>
      </div>
